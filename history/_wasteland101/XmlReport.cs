...
namespace Wasteland 
{
	class XmlReport 
	{
		...
		public static string Generate(Faction f) 
		{
			...
			foreach (string s1 in faction.Events) 
			{
				string s = s1;
				if (s.IndexOf("|") >= 0) 
				{
        ...
        XmlElement elEvent = doc.CreateElement("event");
				...
				if (s.IndexOf('(') >= 0) 
				{
					...
					if (MyStrings.IsNumber(n)) 
					{
						Person p = Person.Get(Convert.ToInt32(n));
						if (p.Faction != faction)
							elEvent.SetAttribute("person", s.Substring(0, s.IndexOf('(')-1));
					}
					else
						elEvent.SetAttribute("person", s.Substring(0, s.IndexOf('(')-1));
					elEvent.SetAttribute("n", n);
					elEvent.SetAttribute("text", s.Substring(s.IndexOf(')') + 3));
				}
				else
					elEvent.SetAttribute("text", s);
			}
			...
			foreach (Region r in Map.Regions) 
			{
				...
				Map.Turn++;
				if (faction.Options.Lang == Lang.En)
					elRegion.SetAttribute("weather", r.Weather.FullNameEn);
				else
					elRegion.SetAttribute("weather", r.Weather.FullNameRu);
