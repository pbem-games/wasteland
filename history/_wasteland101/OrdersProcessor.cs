...
namespace Wasteland 
{
  class OrdersProcessor 
  {
    ...
    private static void DoQuit(Person p, Order order) 
    {
      ...
      if (chosen == null)
				throw new ProcessException("Chosen already dead.|Избранный уже погиб.");
							...
							&& Constants.Random(100) < p.Man.MutatePercent) 
						{
							...
							if (!p.Man.IsMonster) 
							{
								if (Constants.Random(100) < 3)
									p.Description = "ХхХХоччЧу ЕссСсть МмммозггГгии...";
								else if (Constants.Random(100) < 3)
									p.Description = "Господи! Это что у меня тут такое выросло?!";
							}
						}
						else 
						{
					...
					if (Constants.Random(100) < 10) 
					{
						...
						if (chosen != null) 
						{
							if (p.Faction.Options.Lang == Lang.En) 
								...
								p.Description = "Ненавижу тебя, " + chosen.Name + "!";
						}
					}
      ...
      foreach (Person p in Person.List) 
      {
        ...
        if (!p.Faction.IsNPC)
          ...
          if (Constants.Random(100) < 4)
						p.Description = "Что-то мне нехорошо...";
          else if (Constants.Random(100) < 1)
						p.Description = "Меня тошнит...";
				}
				else
					p.Description = "";
				...
				else 
				{
					...
					if (!p.Man.IsMonster && Constants.Random(100) < 5) 
					{
						if (p.Man.RadiationFrom > 0)
							p.Description = "Здесь тепло и светло.";
						else
							p.Description = "Отличное местечко.";
					}
				}
      }
    }
      ...
      foreach (Region r in Map.Regions) 
      {
        ...
        foreach (Person p in persons) 
        {
          ...
          if (p.Killed || (p.Faction.IsNPC && Constants.Random(100) > p.Man.Aggression))
            continue;
					PersonArrayList targets = new PersonArrayList();
