...
namespace Wasteland 
{
	class Battle
	{
		...
		private ItemArrayList Spoils = new ItemArrayList();
		...
		public Battle(PersonArrayList attackers, Person defender, Region region) 
		{
			Init(attackers, defender, region);
		}

		private void Init(PersonArrayList attackers, Person defender, Region region) 
		{
			...
			// Add region patrol
			foreach (Person patrol in defender.Region.Persons)
				if (patrol.Leader == null && patrol.Patrolling
					&& patrol.AttitudeTo(defender) == Attitude.Ally
					&& !defender.GetTeamAndLeader().Contains(patrol))
					foreach (Person soldier in patrol.GetTeamAndLeader()) 
					{
						if (check_persons.Contains(soldier))
							throw new Exception("Soldier already added");
						check_persons.Add(soldier);

						Soldiers.Add(new Soldier(this, soldier, Side.Defender));
					}
			...
			BattleReport.Add("");
			BattleReport.Add("Attackers:|јтакующие:");
			for (int i = 0; i < FirstDefender; i++)
				PrintSoldier((Soldier)Soldiers[i]);
			BattleReport.Add("");
			BattleReport.Add("Defenders:|«ащитники:");
			...
			{
				BattleReport.Add("The battle is a draw.|Ѕой закончен вничью.");
		...
		private void PrintSoldier(Soldier soldier) 
		{
			...
			if (soldier.Weapon != null) 
			{
				...
				if (soldier.Weapon.Ammo != null)
					s += " (зар€ды: " + soldier.Ammo.ToString() + ")";
			}
			if (soldier.Armor != null && !soldier.Armor.IsMan)
      ...
      ArrayList reflexes = new ArrayList();
			...
			for (int turn = 1; turn <= 50; turn++) 
			{
				...
				BattleReport.Add("");
				BattleReport.Add(String.Format("Round {0}{1}|–аунд {0}{2}", 
					turn.ToString(), 
            ...
            int idx = Constants.Random(acting.Count);
				...
				if (turn > 1) 
				{
					if (!attackers_moved) 
					{
						BattleReport.Add("Attackers panicked and ran away.|јтакующие запаниковали и убегают.");
						for (int i = 0; i < FirstDefender; i++)
							((Soldier)Soldiers[i]).Flee = true;
					}
					if (!defenders_moved) 
					{
						BattleReport.Add("Defenders panicked and ran away.|«ащитники запаниковали и убегают.");
						for (int i = FirstDefender; i < Soldiers.Count; i++)
							((Soldier)Soldiers[i]).Flee = true;
					}
				}
		...
		private void MoveSoldier(Soldier s, int round) 
		{
			...
			if (s.Flee) 
			{
				OOASoldier(s);
				s.FleedAway = true;
				BattleReport.Add(String.Format("{0} ran away from combat.|{1} убегает из бо€.",
					s.Person.ToString(Lang.En), s.Person.ToString(Lang.Ru)));
				return;
			}
		...
		private bool OutOfAmmo(Soldier s) 
		{
			...
			{
				BattleReport.Add(String.Format("{0} is out of ammo.|{1}: кончились зар€ды.",
					s.Person.ToString(Lang.En), s.Person.ToString(Lang.Ru)));
				s.Flee = true;
				return true;
			}
		...
		private void ExplodeTank(Soldier s) 
		{
			...
			string report = String.Format(@"{0} -> {1}: {4}|{2} -> {3}: {5}", 
				s.Person.ToString(Lang.En), target.ToString(Lang.En),
				s.Person.ToString(Lang.Ru), target.ToString(Lang.Ru), "{0}", "{1}");

			// Roll to hit
			if (!RollToHit(s, report))
				return;

			// Target is hit, explode
			BattleReport.Add(String.Format(report, "object exploded", "объект взорван"));
			target.Damage();
      ...
      // Pick target
			...
			{
				BattleReport.Add(String.Format(report, "hit for " + s.Weapon.HP.ToString() +
					" hp", "выбито хитов: " + s.Weapon.HP.ToString()));
			}
			else 
			{
				BattleReport.Add(String.Format(report, "object damaged", "объект повреждЄн"));
				target.Damage();
        ...
        // Pick target
			...
			string report = String.Format(@"{0} -> {1}: {4}|{2} -> {3}: {5}", 
				s.Person.ToString(Lang.En), target.Person.ToString(Lang.En),
				s.Person.ToString(Lang.Ru), target.Person.ToString(Lang.Ru), "{0}", "{1}");
			...
			if (!s.Weapon.Ranged && (target.Weapon != null && !target.Weapon.Ranged)) 
			{
				...
				if (parry_roll == 99) 
				{
					BattleReport.Add(String.Format(report, "critical block", "критический блок"));
					return;
				}
				if (parry_roll == 0) 
					BattleReport.Add(String.Format(report, "critical block fail", "критический провал блока"));
				else
				{
					if (target.SkillLevel > parry_roll) 
					{
						BattleReport.Add(String.Format(report, "blocked", "удар блокирован"));
						return;
					}
				}
			}
			...
			if (!s.Weapon.AntiTank) 
			{
				...
				if (save_roll == 99) 
				{
					BattleReport.Add(String.Format(report, "critical save", "критическа€ защита"));
					return;
				}
				if (save_roll == 0) 
					BattleReport.Add(String.Format(report, "critical save fail", "критический провал защиты"));
				else
				{
					if (s.Weapon != null)
					...
					{
						BattleReport.Add(String.Format(report, "armor save", "защита бронЄй"));
						return;
					}
				}
      ...
      KillTarget(s, target);
		...
		private void KillTarget(Soldier s, Soldier target) 
		{
			OOASoldier(target);
			...
			if (killed) 
			{
				...
				BattleReport.Add(String.Format("{0} killed {1}.|{2} убивает персонажа {3}.",
					s.Person.ToString(Lang.En),	target.Person.ToString(Lang.En),
					s.Person.ToString(Lang.Ru),	target.Person.ToString(Lang.Ru)));
			}
			else
				BattleReport.Add(String.Format("{0} incapacitated {1}.|{2} выводит из стро€ персонажа {3}.",
					s.Person.ToString(Lang.En),	target.Person.ToString(Lang.En),
					s.Person.ToString(Lang.Ru),	target.Person.ToString(Lang.Ru)));
		}

		private bool RollToHit(Soldier s, string report) 
		{
			int tohit_roll = Constants.Random(100);
			if (tohit_roll == 0) 
			{
				BattleReport.Add(String.Format(report, "critical miss", "критический промах"));
				return false;
			}
			if (tohit_roll == 99) 
				BattleReport.Add(String.Format(report, "critical hit", "критическое попадание"));
			else
			{
				tohit_roll -= s.Weapon.HitModifier;
				if (tohit_roll >= s.SkillLevel) 
				{
					BattleReport.Add(String.Format(report, "missed", "промах"));
					return false;
				}
			}
			return true;
		...
		private bool RollToWound(Soldier s, string report) 
		{
			int towound_roll = Constants.Random(100);
			if (towound_roll == 0) 
			{
				BattleReport.Add(String.Format(report, "critical wound fail", "критический провал ранени€"));
				return false;
			}
			if (towound_roll == 99) 
				BattleReport.Add(String.Format(report, "critical wound", "критическое ранение"));
			else
			{
				int towound;
				...
				{
					BattleReport.Add(String.Format(report, "no wound", "нет ранени€"));
					return false;
				}
			}
			return true;
