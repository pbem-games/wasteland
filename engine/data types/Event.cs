		...
		private static Hashtable Messages
		{
			...
			get 
			{
				if (_messages == null) 
				{
					...
					_messages.Add(Code.Promoted, new EventMessage(
						"Promoted {0}.",
						"Продвигает персонажа {0}."
						));
					_messages.Add(Code.PromotedBy, new EventMessage(
						"Promoted by owner.",
						"Владелец продвинул на первое место."
						));
					...
					_messages.Add(Code.PromoteLeader, new EventMessage(
						"Team leader must be promoted.",
						"Продвигать нужно командира бригады."
						));
				}
				return _messages;
			}
