...
namespace Wasteland
{
	public class Person : ILocalized
	{
		...
		public bool IsDangerouslyInsane() 
		{
			int insanity = this.Insanity;
			if (this.Faction == Faction.TheGreatest)
				insanity--;
      return (insanity >= Constants.DangerousInsanity);
		}
      ...
      this.Orders.Add(leader_order);
		...
		public Attitude AttitudeTo(Person p) 
		{
			...
			if (Faction.IsNPC) 
			{
				if (p.IsDangerouslyInsane() && !p.Faction.IsNPC)
					return Attitude.Hostile;
				else
